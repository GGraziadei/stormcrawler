...
These are Apache Storm settings rather than StormCrawler ones, but they have a direct effect on a crawl
topology and are set in the `crawler-conf.yaml` generated by the archetypes. See the
https://storm.apache.org/releases/current/Performance.html[Storm performance tuning guide] for the full list.

[cols="1,1,3", options="header"]
|===
| key | default value | description

| topology.message.timeout.secs | 30 | Time given to a tuple tree to be fully acked before the spout is asked to fail it. Crawl topologies set it to several minutes as a URL can wait for a long time in the fetcher queues.
| topology.max.spout.pending | null | Maximum number of tuples emitted by a spout that can be pending on a single spout task at any time. Bounds the number of URLs in flight and, indirectly, the size of the fetcher queues.
| topology.producer.batch.size | 1 | Number of tuples an executor buffers before handing them to the destination executor. Larger batches reduce the per-tuple cost of the internal queues at the expense of latency. Must not exceed half of `topology.executor.receive.buffer.size` and should stay below `topology.max.spout.pending`.
| topology.producer.batch.dynamic | false | Storm 3.0+. When true the batch size adapts at runtime between 1 and `topology.producer.batch.size`: it shrinks toward 1 when batches are flushed partially filled (light load) and grows back under sustained load. Has no effect unless `topology.producer.batch.size` is greater than 1. The archetypes enable it.
| topology.batch.flush.interval.millis | 1 | How often partially filled batches are flushed, so that a batch never waits longer than this for more tuples.
|===
